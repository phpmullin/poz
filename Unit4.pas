...
type
  TAddUr = class(TForm)
    ...
    KCombo_kol_sobst: TcxComboBox;
    KCombo_sobstv_kids: TcxComboBox;
    sDateEdit1: TcxDateEdit;
    Date_Sozvon_komnata: TcxDateEdit;
    Date_sozvon: TcxDateEdit;
    Label17: TLabel;
    ZDate_sozvon: TcxDateEdit;
    Label18: TLabel;
    KDate_sozvon: TcxDateEdit;
    ...
    procedure Edit_S_obKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure KCombo_kol_sobstPropertiesChange(Sender: TObject);
...
procedure TAddUr.KCombo_kol_sobstPropertiesChange(Sender: TObject);
 var kol: integer;
begin
if KCombo_sobstv_kids.text='' then KCombo_sobstv_kids.text:='0';
if KCombo_sobstv_kids.ItemIndex=0 then  kol:=0 else kol:=KCombo_sobstv_kids.ItemIndex;
if KCombo_kol_sobst.ItemIndex<=kol then KCombo_sobstv_kids.ItemIndex:=KCombo_kol_sobst.ItemIndex;
if KCombo_kol_sobst.ItemIndex=0 then KCombo_sobstv_kids.ItemIndex:=0;
end;
...
//---------------Добавление в коммерческую таблицу------------- //
...
',NaznachenieID,	Elektrika,	Kommentarii,	Klass_houseID,	Vsobstven_let,	Tip_zdaniyaID,	kolSobV,	kolSobNL, Bathroom_kol, Balkon_kol, date_sozvon)' +
...
',:p_NaznachenieID,	:p_Elektrika,	:p_Kommentarii,	:p_Klass_houseID,	:p_Vsobstven_let,	:p_Tip_zdaniyaID,	:p_kolSobV,	:p_kolSobNL, :p_Bathroom_kol, :p_Balkon_kol, :p_date_sozvon)';
...
query.ParamByName('p_date_sozvon').asDateTime:=KDate_sozvon.Date;
...
//---------------Добавление в загородную таблицу------------- //
...
+'Vsobstven_let,Kommentarii,Naseleniy_punkt,Stanciya_JD,Udal_stancya,S_uchastka,S_doma,date_sozvon)'+
...
+':p_Vsobstven_let,:p_Kommentarii,:p_Naseleniy_punkt,:p_Stanciya_JD,:p_Udal_stancya,:p_S_uchastka,:p_S_doma,:p_date_sozvon)';
...
query.ParamByName('p_date_sozvon').asDateTime:=ZDate_sozvon.Date;
...
//---------------Добавление в риелторскую таблицу------------- //
...
//---------------Добавление в юридическую таблицу------------- //
...
query.ParamByName('t').asString:=sTimePicker1.Text;
query.ParamByName('d').asDateTime:=sDateEdit1.Date;
query.ParamByName('r').asInteger:=Trunc(Stoimost_rub.value);
query.ParamByName('v').AsInteger:=reklama.EditValue;
query.ParamByName('f').asString:=Fam.text;
query.ParamByName('n').asString:=Name1.text;
query.ParamByName('o').asString:=Otch.text;
query.ParamByName('tt').asString:=sMaskEdit1.text;
query.ParamByName('e').asString:=email.text;
if sradiogroup1.ItemIndex=-1 then query.ParamByName('p').AsInteger:=0;
if sradiogroup1.ItemIndex=0 then query.ParamByName('p').AsInteger:=1;
if sradiogroup1.ItemIndex=1 then query.ParamByName('p').AsInteger:=2;
query.ParamByName('td').asString:=Tip_dogovora.EditValue;
query.ParamByName('ot').asString:=Otdel_komun.EditValue;
query.ParamByName('pr').asinteger:=strtoint(Propisano.Text);
query.ParamByName('ds').asDateTime:=Date_sozvon.Date;
query.ParamByName('m').AsInteger:=Manager.EditValue;
query.ParamByName('k').asString:=sMemo1.Text;
query.OpenOrExecute;
query.Close;
...
procedure TAddUr.sDateEdit1Exit(Sender: TObject);
begin
if datetostr((Sender as TcxDateEdit).Date)='' then (Sender as TcxDateEdit).Date:=date;
 label3.Caption:=datetostr((Sender as TcxDateEdit).Date);
end;
